...
# Edit ansible/hosts.ini to add your servers, then deploy (default: 2 instances on port 8080/8081)
ansible-playbook -i ansible/hosts.ini ansible/gofile.yml -e "HOSTS=servers"

# Deploy with custom instances
ansible-playbook -i ansible/hosts.ini ansible/gofile.yml -e "HOSTS=servers" -e @extra_vars.yml
```
...
gofile_list:
  - port: 8080
    dir: /data/public
    extra_args: "-gzip -cache"
  - port: 9000
    dir: /data/uploads
    extra_args: "-upload -uploadSize 100 -auth -authString admin:secret123"
  - port: 9001
    dir: /data/docs
    extra_args: ""
```
...
|----------|---------|-------------|
| `gofile_dir` | `/srv/gofile` | Base installation directory |
| `gofile_version` | `latest` | Release version (e.g., `v1.0.0`) |
| `gofile_using_cur_user` | `true` | Use current ansible user |
| `gofile_user` | `gofile` | Dedicated user (when `gofile_using_cur_user: false`) |
| `gofile_list` | 2 instances | List of instances (port, dir, extra_args) |
